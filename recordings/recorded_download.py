...
async def run(playwright: Playwright) -> None:
    """
    Echotik 数据下载录制脚本（Download）

    功能流程：
    1. 登录 Echotik 网站
    2. 进入面板页面，开始操作
    3. 导航到「商品排行榜 - 热门推广商品（Hot Promoted）」页面
    4. 设置导出200条记录，触发导出并等待文件下载
    5. 导航到「卖家排行榜 - 最佳跨境卖家（Best Cross-border Seller）」页面
    6. 设置导出200条记录，触发导出并等待文件下载
    7. 关闭所有页面和浏览器

    与其他录制脚本的区别：
    - 本脚本使用 expect_download 来捕获实际的文件下载事件
    - 商品部分访问的是 Hot Promoted（热门推广）而非 Top Sold（热销）
    """
    ...
    # 在展开的子菜单中，点击"Hot Promoted"（热门推广商品排行榜）链接
    await page.locator("#arco-menu-0-submenu-inline-1").get_by_role("link", name="Hot Promoted").click()

    # 直接导航到热门推广商品排行榜页面（带有具体的查询参数）
    # time_type=daily 表示按日查看，time_range=20260305 表示日期，page=1 第一页
    ...
    # --- 触发导出并等待文件下载 ---
    # expect_download() 用于捕获浏览器的文件下载事件
    # expect_popup() 用于捕获点击"Export"后弹出的新窗口/标签页
    # 两者嵌套使用：外层等待下载完成，内层等待弹窗出现
    async with page.expect_download() as download_info:
        async with page.expect_popup() as page1_info:
            # 点击"Export"按钮，触发数据导出（会打开新标签页并开始下载）
            await page.get_by_role("button", name="Export").click()
        # 获取弹出的新页面对象（下载页面）
        page1 = await page1_info.value
    # 获取下载对象，可用于获取下载文件的路径、文件名等信息
    ...
    # --- 触发导出并等待文件下载 ---
    # 同样使用 expect_download + expect_popup 嵌套捕获下载事件和弹窗

...
async def run(playwright: Playwright) -> None:
    """
    Echotik 每月数据采集录制脚本（Monthly）

    功能流程：
    1. 登录 Echotik 网站
    2. 进入面板页面，开始操作
    3. 导航到「商品排行榜 - 热销商品（Top Sold）」页面，切换到"Monthly"时间维度
    4. 设置导出200条记录，触发导出并等待下载弹窗
    5. 导航到「卖家排行榜 - 最佳跨境卖家（Best Cross-border Seller）」页面，切换到"Monthly"时间维度
    6. 设置导出200条记录，触发导出并等待下载弹窗
    7. 关闭所有页面和浏览器

    与 recorded_daily.py 的区别：
    - 时间维度选择"Monthly"（月度）而非"Daily"（每日）
    - 卖家页面通过直接 goto 导航而非仅点击菜单链接
    """
    ...
    # 第一次填入邮箱时输入不完整（录制时的实际操作痕迹）
    await page.get_by_role("textbox", name="Email").fill("yr9m6eyds")
    # 重新导航到登录页面（录制时用户刷新了页面重新输入）
    await page.goto("https://www.echotik.live/login")
    # 重新填入完整的登录邮箱地址
    ...
    # time_type=daily 是默认参数，后续会通过点击切换为 Monthly
    # time_range=20260305 表示日期，order=sale_cnt 按销量排序
    ...
    # 点击"Monthly"标签，将时间维度从默认的 Daily 切换为 Monthly（月度）
    ...
    # 直接导航到跨境卖家排行榜页面（带有具体的查询参数）
    # 录制时通过直接URL导航确保页面正确加载
    await page.goto("https://www.echotik.live/shops/leaderboard/cross-border?time_type=daily&time_range=20260305&page=1")

    # 点击"Monthly"标签，将时间维度切换为 Monthly（月度）
    await page.get_by_text("Monthly").click()

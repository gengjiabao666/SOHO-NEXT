...
async def run(playwright: Playwright) -> None:
    """
    Echotik 每周数据采集录制脚本（Weekly）

    功能流程：
    1. 登录 Echotik 网站
    2. 进入面板页面，开始操作
    3. 导航到「商品排行榜 - 热销商品（Top Sold）」页面，切换到"Weekly"时间维度
    4. 设置导出200条记录，触发导出并等待下载弹窗
    5. 导航到「卖家排行榜 - 最佳跨境卖家（Best Cross-border Seller）」页面，切换到"Weekly"时间维度
    6. 设置导出200条记录，触发导出并等待下载弹窗
    7. 关闭所有页面和浏览器

    与 recorded_daily.py 的区别：
    - 时间维度选择"Weekly"（周度）而非"Daily"（每日）
    """
    ...
    # 第一次填入邮箱时输入不完整（录制时的实际操作痕迹，带了一个@符号）
    ...
    # time_type=daily 是默认参数，后续会通过点击切换为 Weekly
    ...
    # 点击"Weekly"标签，将时间维度从默认的 Daily 切换为 Weekly（周度）
    ...
    # 点击"Weekly"标签，将时间维度切换为 Weekly（周度）
    await page.get_by_text("Weekly").click()
